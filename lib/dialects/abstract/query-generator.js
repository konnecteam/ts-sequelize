...
const QueryGenerator = {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.include) {
      for (const include of options.include) {
        ...
        if (limit && (include.required === true || include.mismatch) && (include.association.isMultiAssociation || include.hasMultiAssociation)) {
      ...
      if (options.groupedLimit) {
        ...
        mainQueryItems.push(this.selectFromTableFragment(options, mainTable.model, attributes.main, '(' +
          options.groupedLimit.values.map(value => {
            ...
            } while((subQuerySlicePos = queryResult.indexOf(placeHolder)) !== -1);
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      if (include.subQuery && topLevelInfo.subQuery) {
        ...
        if (include.association instanceof HasMany && include.parent.subQuery) {
    ...
    if (include.through) {
      ...
      const isHasMany = include.association instanceof HasMany;
      ...
      // If has offset & (parent inner join OR mismatch required) & !not a parent hasMany & is hasMany
      const hasLimitRequiredHasMany = topLevelInfo.options.limit && (hasParentRequired || include.hasMultiAssociation) /*include.required*/ && 
      !Model.hasParentHasMany(include) && (isHasMany || include.hasMultiAssociation) ? true : false;
      // BTM has already generated a subquery
      const notFromBTM = !(Model.hasParentBelongsToMany(include, true));

      if (Model.hasTopParentMismatchOrRequired(include) && !(Model.hasParentInSubQuery(include)) && notFromBTM /*&& (include.required === true || include.mismatch)*/ && (hasLimitRequiredHasMany || (topLevelInfo.subQuery && include.subQueryFilter))) {
        const associationWhere = {};
        include.inSubQuery = true;

        const leftPkDo = association.isSingleAssociation ? (association.targetKeyField || association.target.primaryKeyField) : association.identifierField;
        const rightPkDo =  association.isSingleAssociation ? ((association instanceof BelongsTo ? association.foreignKey : association.sourceKeyField) || association.source.primaryKeyField) : (association.sourceKeyField || association.source.primaryKeyField);
        associationWhere[leftPkDo] = {
          [Op.eq]: this.sequelize.literal(`${this.quoteTable(parentTableName.internalAs)}.${this.quoteIdentifier(rightPkDo)}`)
        };
    ...
    if (joinQuery) {
      ...
      if (include.subQuery && topLevelInfo.subQuery && hasParentRequired) {
        if (requiredMismatch) {
  ...
  generateJoin(include, topLevelInfo) {
    ...
    const parentExistInSubQuery = Model.hasParentInSubQuery(include);
    if ( !parentExistInSubQuery && ((topLevelInfo.options.groupedLimit && parentIsTop) || (topLevelInfo.subQuery && include.parent.subQuery && !include.subQuery && Model.hasReqOrParentRequired(include, true)) )) {
      if (parentIsTop) {
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const hasParentInSubQuery = Model.hasParentInSubQuery(include);
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        if (
          ...
          const quotedOrderBy = this.quote(order, model, '->', options);
