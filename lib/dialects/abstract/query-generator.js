  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.include) {
      for (const include of options.include) {
        ...
        if (limit && include.required === true && include.association.isMultiAssociation && include.parent.hasRequired === true) {
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
        ...
        if (include.association instanceof HasMany && include.parent.subQuery) {
          ...
          const hasAttributeKey = include.parent.attributes.find( f => {
            if (f instanceof Array) {
              return f[0] === include.association.sourceKeyField && f[1] === include.association.sourceKeyAttribute;
            } else {
    ...
    if (include.through) {
      ...
      if (include.required === true && (hasLimitRequiredHasMany || (topLevelInfo.subQuery && include.subQueryFilter))) {
    ...
    if (include.subQuery && topLevelInfo.subQuery && (hasParentRequired || include.mismatch)) {
  ...
  generateJoin(include, topLevelInfo) {
    ...
    if ( (topLevelInfo.options.groupedLimit && parentIsTop) || (topLevelInfo.subQuery && include.parent.subQuery && !include.subQuery && Model.hasReqOrParentRequired(include, true)) ) {
      if (parentIsTop) {
