  ...
  suite('select', () => {
    ...
    (function() {
      ...
      testsql({
        ...
        default: 'SELECT [user].* FROM ('+
        [
          '(SELECT [user].[id_user] AS [id], [user].[last_name] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 1 WHERE ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND [project_users].[project_id] = 1)' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT [user].[id_user] AS [id], [user].[last_name] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 5 WHERE ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND [project_users].[project_id] = 5)' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') +sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
        +') AS [user] ORDER BY [subquery_order_0] ASC;',
        oracle: 'SELECT \"user\".* FROM ('+
        [
          '(SELECT "user".id_user AS id, "user".last_name AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 1 WHERE ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND project_users.project_id = 1) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT "user".id_user AS id, "user".last_name AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 5 WHERE ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND project_users.project_id = 5) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
      ...
      testsql({
        ...
        default: 'SELECT [user].* FROM ('+
        [
          '(SELECT [user].[id_user] AS [id], [user].[last_name] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 1 AND [project_users].[status] = 1 WHERE ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND ([project_users].[project_id] = 1 AND [project_users].[status] = 1))' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT [user].[id_user] AS [id], [user].[last_name] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 5 AND [project_users].[status] = 1 WHERE ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND ([project_users].[project_id] = 5 AND [project_users].[status] = 1))' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') +sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
        +') AS [user] ORDER BY [subquery_order_0] ASC;',
        oracle: 'SELECT \"user\".* FROM ('+
        [
          '(SELECT "user".id_user AS id, "user".last_name AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 1 AND project_users.status = 1 WHERE ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND (project_users.project_id = 1 AND project_users.status = 1)) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT "user".id_user AS id, "user".last_name AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 5 AND project_users.status = 1 WHERE ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND (project_users.project_id = 5 AND project_users.status = 1)) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
        +') \"user\" ORDER BY subquery_order_0 ASC;'
      });
      ...
      testsql({
        ...
        default: 'SELECT [user].* FROM ('+
        [
          '(SELECT [user].[id_user] AS [id], [user].[id_user] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 1 WHERE [user].[age] >= 21 AND ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND [project_users].[project_id] = 1)' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT [user].[id_user] AS [id], [user].[id_user] AS [subquery_order_0], [project_users].[user_id] AS [project_users.userId], [project_users].[project_id] AS [project_users.projectId] FROM [users] AS [user] INNER JOIN [project_users] AS [project_users] ON [user].[id_user] = [project_users].[user_id] AND [project_users].[project_id] = 5 WHERE [user].[age] >= 21 AND ( SELECT [user_id] FROM [project_users] AS [project_users] WHERE ([project_users].[user_id] = [user].[id] AND [project_users].[project_id] = 5)' + (current.dialect.name === 'mssql' ? ' ORDER BY [project_users].[user_id]' : '') + sql.addLimitAndOffset({ limit: 1 }) + ' ) IS NOT NULL ORDER BY [subquery_order_0] ASC'+ (current.dialect.name === 'mssql' ? ', [user].[id_user]' : '') +sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
        +') AS [user] ORDER BY [subquery_order_0] ASC;',
        oracle: 'SELECT \"user\".* FROM ('+
        [
          '(SELECT "user".id_user AS id, "user".id_user AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 1 WHERE "user".age >= 21 AND ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND project_users.project_id = 1) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') + sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')',
          '(SELECT "user".id_user AS id, "user".id_user AS subquery_order_0, project_users.user_id AS "project_users.userId", project_users.project_id AS "project_users.projectId" FROM users "user" INNER JOIN project_users project_users ON "user".id_user = project_users.user_id AND project_users.project_id = 5 WHERE "user".age >= 21 AND ( SELECT user_id FROM project_users project_users WHERE (project_users.user_id = "user".id AND project_users.project_id = 5) ORDER BY user_id OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) IS NOT NULL ORDER BY subquery_order_0 ASC'+ (current.dialect.name === 'oracle' ? ', id_user' : '') +sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] })+')'
        ].join(current.dialect.supports['UNION ALL'] ?' UNION ALL ' : ' UNION ')
        +') \"user\" ORDER BY subquery_order_0 ASC;'
      });
    }());
