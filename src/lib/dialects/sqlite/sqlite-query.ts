...
import * as connectionManager from './sqlite-connection-manager';
...
export class SqliteQuery extends AbstractQuery {
   ...
   * @hidden
   */
  private _collectModels(include : any[], prefix? : string) : {} {
    const ret = {};
      ...
      this.database.serialize(() => {
        const executeSql = () => {
          if (this.sql.indexOf('-- ') === 0) {
            ...
            resolve(new Promise((promiseResolve, reject) => {
              const query = this;
                  ...
                  if (query.sql.indexOf('sqlite_master') !== -1) {
                    ...
                    if (!query.options.raw) {
                      ...
                      results = results.map(mapResult => {
                        return _.mapValues(mapResult, (value, name) => {
                          let model;
                          if (name.indexOf('.') !== -1) {
   ...
   * @hidden
   */
  private parseConstraintsFromSql(sql : string) : {} {
    let constraints : any = sql.split('CONSTRAINT ');
   ...
   * @hidden
   */
  private applyParsers(type : string, value : any) : any {
    if (type.indexOf('(') !== -1) {
  ...
  public formatError(err : { code? : string, message? : string }) : Error {
   ...
   * @hidden
   */
  private getDatabaseMethod() : string {
