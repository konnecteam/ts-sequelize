    ...
    describe('HasMany', () => {
      describe('addDAO / getModel', () => {
        beforeEach(function() {
          ...
          this.User = this.sequelize.define('User' + config.rand(), { name: new DataTypes.STRING() });
          this.Task = this.sequelize.define('Task' + config.rand(), { name: new DataTypes.STRING() });
          this.users = null;
          this.tasks = null;
        ...
        it('should correctly add an association to the dao', function() {
          ...
          return self.user.getTasks().then(tasks => {
        ...
        it('should correctly remove associated objects', function() {
          ...
          return this.sequelize.sync({ force: true }).then(() => {
            return self.User.bulkCreate(users).then(() => {
              return self.Task.bulkCreate(tasks).then(() => {
                return self.User.findAll().then(_users => {
                  return self.Task.findAll().then(tasks1 => {
                    self.user = _users[0];
                    self.task = tasks1[0];
                    self.users = _users;
                    self.tasks = tasks1;

                    return self.user.getTasks().then(tasks2 => {
                      expect(tasks2).to.have.length(0);
                      return self.user.setTasks(self.tasks).then(() => {
                        return self.user.getTasks().then(tasks3 => {
                          expect(tasks3).to.have.length(self.tasks.length);
                          return self.user.removeTask(self.tasks[0]).then(() => {
                            return self.user.getTasks().then(tasks4 => {
                              expect(tasks4).to.have.length(self.tasks.length - 1);
                              return self.user.removeTasks([self.tasks[1], self.tasks[2]]).then(() => {
                                return self.user.getTasks().then(tasks5 => {
                                  expect(tasks5).to.have.length(self.tasks.length - 3);
                                });
                              });
                            });
                          });
