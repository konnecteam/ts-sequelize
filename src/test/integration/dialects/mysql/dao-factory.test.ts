  ...
  describe('[MYSQL Specific] DAOFactory', () => {
    describe('constructor', () => {
      ...
      it('adds deletedAt if paranoid', function() {
        ...
        expect(this.sequelize.getQueryInterface().QueryGenerator.attributesToSQL(User.rawAttributes)).to.deep.equal({id: 'INTEGER NOT NULL auto_increment PRIMARY KEY',
      ...
      it('omits blobs fields with defaultValues', function() {
        const User = this.sequelize.define('User' + config.rand(), {name: {type: new DataTypes.STRING().BINARY, defaultValue: 'helloworld'}});
