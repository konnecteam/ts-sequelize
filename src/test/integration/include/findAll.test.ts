...
describe(Support.getTestDialectTeaser('Include'), () => {
  let models : { [key : string] : Model<ItestInstance, ItestAttribute> };
  describe('findAll', () => {
    beforeEach(function() {
    ...
    it('should work on a nested set of relations with a where condition in between relations', function() {
      const User = current.define<ItestInstance, ItestAttribute>('User', {});
      const SubscriptionForm = current.define<ItestInstance, ItestAttribute>('SubscriptionForm', {});
      const Collection = current.define<ItestInstance, ItestAttribute>('Collection', {});
      const Category = current.define<ItestInstance, ItestAttribute>('Category', {});
      const SubCategory = current.define<ItestInstance, ItestAttribute>('SubCategory', {});
      const Capital = current.define<ItestInstance, ItestAttribute>('Capital', {});
    ...
    it('should accept nested `where` and `limit` at the same time', function() {
      ...
      const Set = current.define<ItestInstance, ItestAttribute>('Set', {
        title: new DataTypes.STRING()
      });
      ...
      return current.sync({force: true}).then(() => {
        return Promise.join(
          ...
          return Promise.join(
            sets[0].addLinkedData('Product', [products[0], products[1]]),
            products[0].addLinkedData('Tag', tags[0], {priority: 1}).then(() => {
              return products[0].addLinkedData('Tag', tags[1], {priority: 2});
            }).then(() => {
              return products[0].addLinkedData('Tag', tags[2], {priority: 1});
            }),
            products[1].addLinkedData('Tag', tags[1], {priority: 2}).then(() => {
              return products[2].addLinkedData('Tag', tags[1], {priority: 3});
            }).then(() => {
              return products[2].addLinkedData('Tag', tags[2], {priority: 0});
            })
          );
        }).then(() => {
          return Set.findAll({
      ...
      return current.sync({force: true}).then(() => {
        return (Promise as any).join(
          ...
          return Promise.map(as, a => {
            return (a as ItestInstance).setLinkedData('b', b);
          });
        }).then(() => {
          return A.findAll({
            include: [
    ...
    it('should support ordering with only belongsTo includes', function() {
      ...
      const Order = current.define<ItestInstance, ItestAttribute>('Order', {position: new DataTypes.INTEGER()});
      ...
      return current.sync().then(() => {
        return Promise.props({
          ...
          return Promise.join(
            user1.setLinkedData({ model : 'Item', associationAlias : 'itemA' }, item1),
            user1.setLinkedData({ model : 'Item', associationAlias : 'itemB' }, item2),
            user1.setLinkedData('Order', order3),
            user2.setLinkedData({ model : 'Item', associationAlias : 'itemA' }, item3),
            user2.setLinkedData({ model : 'Item', associationAlias : 'itemB' }, item4),
            user2.setLinkedData('Order', order2),
            user3.setLinkedData({ model : 'Item', associationAlias : 'itemA' }, item1),
            user3.setLinkedData({ model : 'Item', associationAlias : 'itemB' }, item4),
            user3.setLinkedData('Order', order1)
          );
        }).then(() => {
          return User.findAll({
            include: [
    ...
    it('should include attributes from through models', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return Promise.join(
            results.products[0].addLinkedData('Tag', results.tags[0], { through: {priority: 1}}),
            results.products[0].addLinkedData('Tag', results.tags[1], { through: {priority: 2}}),
            results.products[1].addLinkedData('Tag', results.tags[1], { through: {priority: 1}}),
            results.products[2].addLinkedData('Tag', results.tags[0], { through: {priority: 3}}),
            results.products[2].addLinkedData('Tag', results.tags[1], { through: {priority: 1}}),
            results.products[2].addLinkedData('Tag', results.tags[2], { through: {priority: 2}})
          );
        }).then(() => {
    ...
    it('should support a required belongsTo include', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return results.users[2].setLinkedData('Group', results.groups[1]);
        }).then(() => {
          return User.findAll({
            include: [
    ...
    it('should be possible to extend the on clause with a where option on a belongsTo include', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return Promise.join(
            results.users[0].setLinkedData('Group', results.groups[1]),
    ...
    it('should be possible to define a belongsTo include as required with child hasMany not required', function() {
      const Address = current.define<ItestInstance, ItestAttribute>('Address', { active: new DataTypes.BOOLEAN() });
      const Street = current.define<ItestInstance, ItestAttribute>('Street', { active: new DataTypes.BOOLEAN() });
        ...
        return Promise.props({
          ...
          return Promise.join(
            ...
            Promise.map(results.groups, group => {
              return group.setLinkedData('Category', results.categories);
            }) as any
          );
        }).then(() => {
          return User.findAll({
    ...
    it('should be possible to define a belongsTo include as required with child hasMany with limit and aliases', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return Promise.join(
            results.users[0].setLinkedData({ model : 'Group', associationAlias : 'Team' }, results.groups[1]),
            results.users[1].setLinkedData({ model : 'Group', associationAlias : 'Team' }, results.groups[0]),
            Promise.map(results.groups, group => {
              return group.setLinkedData({ model : 'Category', associationAlias : 'Tags' }, results.categories);
            }) as any
          );
        }).then(() => {
          return User.findAll({
    ...
    it('should be possible to extend the on clause with a where option on a hasOne include', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return Promise.join(
            results.users[1].setLinkedData({ model : 'Project', associationAlias : 'LeaderOf' }, results.projects[1]),
            results.users[0].setLinkedData({ model : 'Project', associationAlias : 'LeaderOf' }, results.projects[0])
          );
        }).then(() => {
          return User.findAll({
            include: [
    ...
    it('should be possible to extend the on clause with a where option on a hasMany include with a through model', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.props({
          ...
          return Promise.join(
            results.products[0].addLinkedData('Tag', results.tags[0], {priority: 1}),
            results.products[0].addLinkedData('Tag', results.tags[1], {priority: 2}),
            results.products[1].addLinkedData('Tag', results.tags[1], {priority: 1}),
            results.products[2].addLinkedData('Tag', results.tags[0], {priority: 3}),
            results.products[2].addLinkedData('Tag', results.tags[1], {priority: 1}),
            results.products[2].addLinkedData('Tag', results.tags[2], {priority: 2})
          );
        }).then(() => {
          return Product.findAll({
            include: [
          ...
          return Promise.each([0, 1, 2, 3, 4], i => {
            return Promise.props({
              ...
              return Promise.join(
                ...
                results.user.setLinkedData('Product', [
                  results.products[i * 2 + 0],
                  results.products[i * 2 + 1],
                ]) as any,
                Promise.join(
                  results.products[i * 2 + 0].setLinkedData('Tag', [
                    tags[0],
                    tags[2],
                  ]),
                  results.products[i * 2 + 1].setLinkedData('Tag', [
                    tags[1],
                  ]),
                  results.products[i * 2 + 0].setLinkedData({ model : 'Tag', associationAlias : 'Category' }, tags[1])
                ),
                Price.bulkCreate([
    ...
    it('should be possible use limit, attributes and a where on a belongsTo with additional hasMany includes', function() {
      return this.fixtureA().then(() => {
        return models.Product.findAll({
          ...
          limit: 3,
          order: [
            [current.col(models.Product.name + '.id'), 'ASC'],
          ]
        }).then(products => {
    ...
    it('should be possible to have the primary key in attributes', function() {
      const Parent = current.define<ItestInstance, ItestAttribute>('Parent', {});
      const Child1 = current.define<ItestInstance, ItestAttribute>('Child1', {});
      ...
      return current.sync({force: true}).then(() => {
        ...
        return parent.addLinkedData('Child1', child).then(() => {
          return parent;
        });
      }).then(parent => {
        return Child1.find({
    ...
    it('should be possible to turn off the attributes for the through table', function() {
      return this.fixtureA().then(() => {
        return models.Product.findAll({
          attributes: ['title'],
          include: [
            {model: models.Tag, through: {attributes: []}, required: true},
          ]
        }).then(products => {
          products.forEach(product => {
            expect(product.Tags.length).to.be.ok;
    ...
    it('should be possible not to include the main id in the attributes', function() {
      ...
      const Album = current.define<ItestInstance, ItestAttribute>('Album', {
        id: {
          type: new DataTypes.BIGINT(),
          primaryKey: true,
          autoIncrement: true
    ...
    it('should still pull the main record(s) when an included model is not required and has where restrictions without matches', function() {
      const A = current.define<ItestInstance, ItestAttribute>('a', {name: new DataTypes.STRING(40)});
      const B = current.define<ItestInstance, ItestAttribute>('b', {name: new DataTypes.STRING(40)});
      ...
      return current.sync({force: true}).then(() => {
        return Promise.join(
          ...
          return Promise.map(posts.slice(1, 3), post => {
            return post.createLinkedData<ItestInstance, ItestAttribute>('category', {slug: 'food'});
          });
        }).then(() => {
          return Post.findAll({
            limit: 2,
    ...
    it('should work on a nested set of required 1:1 relations', function() {
      ...
      const UserPerson = current.define<ItestInstance, ItestAttribute>('UserPerson', {
        PersonId: {
          type: new DataTypes.INTEGER(),
          primaryKey: true
        },
    ...
    it('should be able to order on the main table and a required belongsTo relation with custom tablenames and limit ', function() {
      ...
      return current.sync({force: true}).then(() => {
        return Promise.join(
          ...
          return Promise.join(
            albertsen.setLinkedData('Company', company1),
            zenith.setLinkedData('Company', company2),
            hansen.setLinkedData('Company', company2)
          );
        }).then(() => {
          return User.findAll({
            include: [
    ...
    it('should ignore include with attributes: [] (used for aggregates)', function() {
      ...
      const Post_Comments = Post.hasMany(Comment, {as: 'comments'});
      ...
      return current.sync({force: true}).bind(this).then(() => {
        return Post.create({
          ...
          include: [Post_Comments]
        });
      }).then(function() {
        return Post.findAll({
          attributes: [
            [current.fn('COUNT', current.col('comments.id')), 'commentCount'],
          ],
          include: [
            {association: Post_Comments, attributes: []},
          ],
          group: [
            'Post.id',
          ]
    ...
    it('Should return posts with nested include with inner join with a m:n association', function() {
      ...
      const TaggableSentient = current.define<ItestInstance, ItestAttribute>('TaggableSentient', {
        nametag: {
          type: new DataTypes.STRING(),
          primaryKey: true
        }
      ...
      return current.sync({ force: true })
        ...
        .then(entity => Promise.all([
          ...
          entity.addLinkedData({ model : 'TaggableSentient', associationAlias : 'tags' }, 'bob'),
        ]))
        .then(() => Post.findAll({
          include: [{
            model: Entity,
    ...
    it('should be able to generate a correct request with inner and outer join', function() {
      ...
      const ShippingAddress = current.define<ItestInstance, ItestAttribute>('shippingAddress', {
        address: new DataTypes.STRING(),
        verified: new DataTypes.BOOLEAN()
      });

      const Order = current.define<ItestInstance, ItestAttribute>('purchaseOrder', {
        description: new DataTypes.TEXT()
      });

      const Shipment = current.define<ItestInstance, ItestAttribute>('shipment', {
    ...
    it('should be able to generate a correct limit request with BT->BT->BT[WA] then hasMany', function() {
      ...
      const Issue = current.define<ItestInstance, ItestAttribute>('issue', {
        name: new DataTypes.STRING()
      });

      const IssueType = current.define<ItestInstance, ItestAttribute>('issueType', {
        description: new DataTypes.TEXT()
      });

      const IssueTypeOptions = current.define<ItestInstance, ItestAttribute>('issueTypeOpt', {
    ...
    it('should be able to generate a correct limit request with hasMany(hasMany(hasOne))', function() {
      ...
      const Job = current.define<ItestInstance, ItestAttribute>('job', {
        title: new DataTypes.STRING()
      });

      const Share = current.define<ItestInstance, ItestAttribute>('share', {
        description: new DataTypes.TEXT()
      });
      ...
      current.define<ItestInstance, ItestAttribute>('subblock', {
        description: new DataTypes.TEXT()
      });
    ...
    it('should be able to generate a correct limit request with hasMany(hasMany(hasOne(hasMany)))', function() {
      ...
      const SubBlock = current.define<ItestInstance, ItestAttribute>('subblock', {
        description: new DataTypes.TEXT()
      });
    ...
    it('should be able to generate a correct limit request with one hasMany and outer belongsTo with belongsTo', function() {
      ...
      const Address = current.define<ItestInstance, ItestAttribute>('address', {
        address: new DataTypes.STRING(),
        verified: new DataTypes.BOOLEAN()
      });
    ...
    it('should be able to generate a correct limit request with hasMany,!belongsTo(belongsTo(hasMany))', function() {
      ...
      const Relation = current.define<ItestInstance, ItestAttribute>('relation', {
        description: new DataTypes.TEXT()
      });
    ...
    it('should be able to generate a correct limit request with two hasOne then hasMany', function() {
      ...
      const Reseller = current.define<ItestInstance, ItestAttribute>('reseller', {
        name: new DataTypes.STRING()
      });
    ...
    if (dialect === 'postgres' && current.options.native === false || dialect === 'mssql') {
      it('should be able to force a limit resultsSet for avoid memory crash', function() {
        ...
        current.config.dialectOptions.maxRows = 3;
        return current.sync({ force: true })
        .then( () => {
          return Promise.all([
          ...
          .then( cus => {
            ...
            .then( results => {
              ...
              current.config.dialectOptions.maxRows = 4;
              return Customer.findAndCountAll({})
              .then( _results => {
                expect(_results.rows.length).to.equal(4);
                expect(_results.count).to.equal(5);
                current.config.dialectOptions.maxRows = undefined;
              });
            });
          });
        })
    ...
    it('should be able to generate a correct belongsToMany(hasOne(hasOne))', function() {
      ...
      const Ability = current.define<ItestInstance, ItestAttribute>('abilitie', {
        name: new DataTypes.STRING()
      });
    ...
    it('should be able to generate a correct BT,HM with order by BT.XXX and offset', function() {
      const User = current.define<ItestInstance, ItestAttribute>('user', { name: new DataTypes.TEXT() });
      const Address = current.define<ItestInstance, ItestAttribute>('address', { name: new DataTypes.STRING() });
      const Abilities = current.define<ItestInstance, ItestAttribute>('abilitie', { name: new DataTypes.STRING() });
    ...
    it('should be able to generate a correct BT(HM(BT)) with order by and offset', function() {
      const Father = current.define<ItestInstance, ItestAttribute>('father', { name: new DataTypes.TEXT() });
      const User = current.define<ItestInstance, ItestAttribute>('user', { name: new DataTypes.TEXT() });
      const Address = current.define<ItestInstance, ItestAttribute>('address', { name: new DataTypes.STRING() });
      const City = current.define<ItestInstance, ItestAttribute>('city', { name: new DataTypes.STRING() });
    ...
    it('should be able to generate a correct BT(HM(BT(BT))) with order by and offset', function() {
      ...
      const Address = current.define<ItestInstance, ItestAttribute>('address', { name: new DataTypes.STRING() });
      const City = current.define<ItestInstance, ItestAttribute>('city', { name: new DataTypes.STRING() });
      const PostalCode = current.define<ItestInstance, ItestAttribute>('postalcode', { name: new DataTypes.STRING() });
    ...
    it('should be able to generate a correct BT(HM) with order by BT.XXX and offset', function() {
      const User = current.define<ItestInstance, ItestAttribute>('user', { name: new DataTypes.TEXT() });
      const Address = current.define<ItestInstance, ItestAttribute>('address', { name: new DataTypes.STRING() });
      const Factions = current.define<ItestInstance, ItestAttribute>('faction', { name: new DataTypes.STRING() });
