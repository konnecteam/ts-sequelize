    ...
    describe('with enums', () => {
      it('references enum in the right schema #3171', () => {
        expectsql(sql.createTableQuery(FooUser.getTableName(), sql.attributesToSQL(FooUser.rawAttributes), { }), {
          ...
          oracle: "BEGIN EXECUTE IMMEDIATE 'CREATE TABLE foo.users (id NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY, mood NVARCHAR2(255),CONSTRAINT PKfoousersid PRIMARY KEY (id), CHECK (mood IN(''happy'', ''sad'')))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;",
