    ...
    describe('with enums', () => {
      it('references enum in the right schema #3171', () => {
        expectsql(sql.createTableQuery(FooUser.getTableName(), sql.attributesToSQL(FooUser.rawAttributes), { }), {
          ...
          oracle: "BEGIN EXECUTE IMMEDIATE 'CREATE TABLE foo.users (id NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY, mood NVARCHAR2(255),"
          + "CONSTRAINT PKfoousersid PRIMARY KEY (id), CHECK (mood IN(''happy'', ''sad'')))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;",
    ...
    describe('with references', () => {
      ...
      it('references right schema when adding foreign key #9029', () => {
        expectsql(sql.createTableQuery(BarProject.getTableName(), sql.attributesToSQL(BarProject.rawAttributes), { }), {
          ...
          mssql: 'IF OBJECT_ID(\'[bar].[projects]\', \'U\') IS NULL CREATE TABLE [bar].[projects] ([id] INTEGER NOT NULL IDENTITY(1,1) ,'
          + ' [user_id] INTEGER NULL, PRIMARY KEY ([id]), FOREIGN KEY ([user_id]) REFERENCES [bar].[users] ([id]) ON DELETE NO ACTION);',
          oracle: "BEGIN EXECUTE IMMEDIATE 'CREATE TABLE bar.projects (id NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY, user_id INTEGER NULL,CONSTRAINT PKbarprojectsid PRIMARY KEY (id),CONSTRAINT FKbarprojectsuser_id FOREIGN KEY (user_id) "
          + "REFERENCES bar.users (id) )'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;"
        });
      });
    });
